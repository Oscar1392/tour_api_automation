    ...
    it('should create a new user using .then() syntax', () => {
        ...
        .end((err: Error | null, res: Response) =>{
            if (err) {
                ...
                return done(err); // Notify jest that the test has failed
            }
            try {
            expect(res.status).toBe(201);
            expect(res.body.status).toBe('success');
            ...
            expect(typeof res.body.data.user.email).toBe('string');
            expect(res.body.token).toBeDefined();
            done(); // Notify jest that the test has completed successfully
            } catch (error) {
            done(error);
            } // Notify jest that the test has failed
        })     
    });
});


//Homework : Write similar tests for User Login endpoint.
